...
# Тишина от облака сама по себе НЕ признак обрыва — это проверено дорого.
# В записи 16.08.2026 пауза между кадрами облака не превышала 8,1 с, и по
# этому замеру порог сперва поставили в 120 с. Но та запись состояла из
# коротких соединений, где облако говорит сразу после подключения. На живом
# долгом соединении 24.09.2026 облако молчало дольше двух минут подряд, и
# сторож начал рвать исправные сессии каждые пару минут. Поэтому здесь
# только дальний предохранитель, а обрыв ловится по отставанию снимка в
# облаке (data_merge.cloud_lags_behind_device).
UPSTREAM_SILENCE_TIMEOUT = 900
...
READ_CHUNK = 65536
# Пауза между командами одного вызова — как у облака, которое шлёт их по
# одной. Сама по себе склейка причиной отказа не была (см. _send), но
# повторять поведение облака дешевле, чем выяснять, что ещё ему важно.
COMMAND_GAP = 0.3
...
class AtmeexLocalChannel:
    ...
    async def drop_session(self, mac: str) -> None:
        """Разорвать сессию устройства, чтобы оно переподключилось.

        Нужно, когда канал перестал доносить поток до облака: бризер держит
        одно соединение сутками и сам об этом не узнает.
        """
        ...
        writer = session[0]
        _LOGGER.warning(
            "Atmeex: разрываю сессию устройства %s — переподключится и "
            "откроет встречное соединение заново",
            mac,
        )
        with contextlib.suppress(Exception):
            writer.close()
    ...
    async def async_send_params(self, mac: str, params: dict[str, Any]) -> bool:
        ...
        # делает облако. Просить отчёт через get_setp/get_state не нужно:
        # облако после команд их не шлёт, устройство само присылает уставки
        # в ответ на команду и состояние каждые ~5 секунд.
        sent = False
    ...
    def _send(writer: asyncio.StreamWriter, payload: dict[str, Any]) -> None:
        # Облако завершает каждый кадр к устройству переводом строки (сырая
        # запись 16.08.2026: все 18 кадров), и прошивка, судя по всему, читает
        # до него: кадр без \n устройство не выполняло, хотя в остальном он
        # совпадал с облачным байт в байт. Устройство в облако пишет вплотную,
        # без разделителя, — направления устроены по-разному.
